...
After installing CodePush CLI globally, it will be available under `rupush`.
...
```
rupush register <optional: server-url>
```
...
```shell
rupush login <optional: server-url>
```
...
```shell
rupush access-key add "VSTS Integration"
```
...
```shell
rupush login --accessKey <accessKey>
```
...
```shell
rupush access-key patch <accessKeyName> --name "new name" --ttl 10d
```
...
```
rupush app add MyApp-Android
rupush app add MyApp-iOS
```
...
```
rupush app rename <appName> <newAppName>
```
...
```shell
rupush collaborator add <appName> <collaboratorEmail>
```
...
```shell
rupush collaborator rm <appName> <collaboratorEmail>
```
...
```shell
rupush collaborator ls <appName>
```
...
```shell
rupush app transfer <appName> <newOwnerEmail>
```

_NOTE: Just like with the `rupush collaborator add` command, this expects that the new owner has already registered with CodePush using the specified e-mail address._

Once confirmed, the specified developer becomes the app's owner and immediately receives the permissions associated with that role. Besides the transfer of ownership, nothing else about the app is modified (e.g. deployments, release history, collaborators). This means that you will still be a collaborator of the app, and therefore, if you want to remove yourself, you simply need to run the `rupush collaborator rm` command after successfully transferring ownership.
...
```
rupush deployment add <appName> <deploymentName>
```

Just like with apps, you can remove and rename deployments as well, using the following commands respectively:

```
rupush deployment rm <appName> <deploymentName>
rupush deployment rename <appName> <deploymentName> <newDeploymentName>
```
...
```
rupush deployment ls <appName> [--displayKeys|-k]
```
...
2. [React Native](#releasing-updates-react-native) - Performs the same functionality as the general release command, but also handles the task of generating the updated app contents for you (JS bundle and assets), instead of requiring you to run both `react-native bundle` and then `rupush release`.
...
```
rupush release <appName> <updateContents> <targetBinaryVersion>
...
This specifies the name of the CodePush app that this update is being released for. This value corresponds to the friendly name that you specified when originally calling `rupush app add` (e.g. "MyApp-Android"). If you need to look it up, you can run the `rupush app ls` command to see your list of apps.
...
```shell
rupush release-react <appName> <platform>
...
rupush release MyApp-iOS ./CodePush 1.0.0
```
...
```shell
rupush release-react MyApp-iOS ios
```
...
```shell
rupush release-react MyApp-Android android -p "./foo/bar/"
rupush release-react MyApp-Android android -p "./foo/bar/build.gradle"
```
...
```shell
rupush release-react MyApp-iOS ios -p "./foo/bar/MyFile.plist"
```
...
```shell
rupush patch <appName> <deploymentName>
[--label <releaseLabel>]
...
rupush patch MyApp-iOS Production -m

# Increase the rollout for v23 to 50%
rupush patch MyApp-iOS Production -l v23 -rollout 50%
```

### Label parameter

Indicates which release (e.g. `v23`) you want to update within the specified deployment. If ommitted, the requested changes will be applied to the latest release in the specified deployment. In order to look up the label for the release you want to update, you can run the `rupush deployment history` command and refer to the `Label` column.
...
rupush patch MyApp-iOS Staging -t "1.0.0 - 1.0.5"
```
...
```
rupush promote <appName> <sourceDeploymentName> <destDeploymentName>
[--description <description>]
[--label <label>]
[--disabled <disabled>]
[--mandatory]
...
rupush promote MyApp-iOS Staging Production -t "*"
```
...
```
rupush rollback <appName> <deploymentName>
rupush rollback MyApp-iOS Production
```
...
```
rupush rollback MyApp-iOS Production --targetRelease v34
```
...
```
rupush deployment history <appName> <deploymentName>
```
...
```
rupush deployment clear <appName> <deploymentName>
```
